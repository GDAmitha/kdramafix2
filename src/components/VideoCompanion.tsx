      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="space-x-4">
            <Button
              variant="outline"
              onClick={() => handlePlaybackChange(!isPlaying)}
            >
              {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
            </Button>
            <Button
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { type: 'gasp', message: ['*Gasp*', 'I did NOT see', 'that coming! 👀'] },
            { type: 'terrible', message: ['How can anyone', 'be THIS', 'terrible?! 😤'] },
            { type: 'kiss', message: ['JUST KISS', 'ALREADY!!!', '😩❤️'] },
            { type: 'crying', message: ['Sobbing uncontrollably', 'Why am I crying', 'right now!!'] },
            { type: 'next', message: ['I need the next', 'episode right now…', 'Please, I beg!'] },
            { type: 'outfit', message: ['I want', 'that fit!'] },
            { type: 'song', message: ['This song is', 'way too good!'] },
            { type: 'communication', message: ['This could all', 'be solved with', 'better communication.', '🤦‍♀️'] },
            { type: 'cheesy', message: ['It\'s cheesy but…', 'I love it!', '🥰'] }
          ].map(({ type, message }) => (
            <Button 
              key={type}
              variant="outline" 
